...
export async function loadPageDeps(deps: string | string[] | PageDeps | PageDeps[], ctx: GetServerSidePropsContext): Promise<Pages> {
    let _deps: PageDeps[] = [];
    ...
    const sections = await database.from('section')
        .select('theme_page_section.page', 'section.page_section', 'value', 'section.sort')
        .join('theme_page_section', 'section.page_section', 'theme_page_section.id')
        .where('section.visible', true)
        .where(qb => qb.whereIn('theme_page_section.page', pageIds)
    ...
    for (let i = 0; i < sections.length; i++) {
        ...
        const sectionId = section.page_section.replace(`${packageJson.name}-${pageId}-`, '');
