...
export async function getSchema(options?: {
	...
	if (env.CACHE_SCHEMA !== false) {
		...
		try {
			cachedSchema = (await systemCache.get(`schema_${database.client.config.connection.database}`)) as SchemaOverview;
		} catch (err: any) {
		...
		if (cachedSchema) {
			...
			try {
				await systemCache.set(`schema_${database.client.config.connection.database}`, result);
			} catch (err: any) {
