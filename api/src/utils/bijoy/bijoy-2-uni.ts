...
export function ConvertToUnicode(text: string, target = "bijoy"): string {
    const conversionMap: ConversionMap = target === 'bijoy' ? bijoyToUniMap : target === 'somewherein' ? somewhereinToUniMap : baishakhiToUniMap;

    for (const map of conversionMap) {
        text = text.replace(new RegExp(map[0], "g"), map[1]);
    }

    text = ReArrangeUnicodeConvertedText(text);
    text = text.replace(new RegExp("অা", "g"), "আ");
