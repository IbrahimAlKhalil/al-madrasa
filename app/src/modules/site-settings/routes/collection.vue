	...
	>
		...
		>
			...
			<template #headline>
				...
				<v-breadcrumb v-else :items="[{ name: t('collections'), to: '/_settings' }]" />
			</template>
				...
				<template #no-items>
					<v-info :title="t('item_count', 0)" :icon="currentCollection.icon" center>
						...
						<template v-if="createAllowed" #append>
							<v-button :to="`/_settings/${collection}/+`">{{ t('create_item') }}</v-button>
						</template>
					</v-info>
				</template>
			</component>
	...
	setup(props) {
		...
		function useLinks() {
			const addNewLink = computed<string>(() => {
				return `/_settings/${props.collection}/+`;
			});
		...
		function useBookmarks() {
			...
			async function createBookmark(name: string) {
				...
				try {
					...
					router.push(`/_settings/${newBookmark.collection}?bookmark=${newBookmark.id}`);
