<template>
	...
	<private-view v-else :title="title">
		...
		<template #headline>
			<v-breadcrumb
				v-if="collectionInfo.meta && collectionInfo.meta.singleton === true"
				:items="[{ name: t('collections'), to: '/_settings' }]"
			/>
...
export default defineComponent({
	...
	setup(props) {
		...
		async function saveAndQuit() {
			...
			try {
				await save();
				if (props.singleton === false) router.push(`/_settings/${props.collection}`);
			} catch {
		...
		async function saveAndStay() {
			...
			try {
				...
				if (props.primaryKey === '+') {
					...
					router.replace(`/_settings/${props.collection}/${encodeURIComponent(newPrimaryKey)}`);
				}
			} catch {
				// Save shows unexpected error dialog
			}
		...
		async function saveAndAddNew() {
			...
			try {
				...
				if (isNew.value === true) {
					...
					router.push(`/_settings/${props.collection}/+`);
				}
			} catch {
				// Save shows unexpected error dialog
			}
		...
		async function saveAsCopyAndNavigate() {
			try {
				...
				if (newPrimaryKey) router.push(`/_settings/${props.collection}/${encodeURIComponent(newPrimaryKey)}`);
			} catch {
				// Save shows unexpected error dialog
			}
		}
